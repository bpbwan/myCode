...
//mbuf[0~3]  �������mbuf[4~7] �е� ���   //mbuf[8~11] �ײ����
...
CUntitled::CUntitled()
{
	  ...
	  start_point =220; //  ���ҿ�ʼ���ߵ�λ��
	  Lstart_point = 80; //  ���ҿ�ʼ���ߵ�λ��
	  Rstart_point = 220;
...
{	//  P=-40;               //  ����ͷ��װƫ��ˮƽ����  ����߸��� 
	 ...
	 //high=850;            //   ������ѧ����������ˮƽ����ĸ߶�
	 //Vision=115;          //  ������ӽ�  
	 //Pu=0.05;             //  ��������ƫ��CCDˮƽ ʹͼ������Ϊ+  0.01
	 //Pv=0.00;             //  ��������ƫ��CCD��ֱ ʹͼ������Ϊ+  0.01
	 //x_angle=33.5;        //  �����X ����ˮƽ�Ƕ�
	 //y_angle=4.0;
	 //z_angle=2.0;
	 ...
	 high=500;            //   ������ѧ����������ˮƽ����ĸ߶�
	 Vision=180;          //  ������ӽ�  
	 Pu=0.08;             //  ��������ƫ��CCDˮƽ ʹͼ������Ϊ+  0.01
	 Pv=0.03;             //  ��������ƫ��CCD��ֱ ʹͼ������Ϊ+  0.01
	 x_angle=24;        //  �����X ����ˮƽ�Ƕ�
	 y_angle=1.0;
	 z_angle=0.0;
}
void CUntitled::init_Camera_Module_insidePm()
{
	...
	//  fccd  = Dccd/(4.0*sin((2*pi*170/4)/360.0))      //   1.70; �����Ľ��� 6.00
	 fhccd = Hccd/(4.0*sin((2*pi*Vision/4)/360.0));      //   ˮƽ�ӽ�*1.8 fhccd = Hccd/(4.0*sin( (2*pi*128/2)/360.0 ))
...
void CUntitled::init_Camera_Module_OutPm()
{
	...
	//   �����ģ�� �ⲿ����
	 Zccd  =high;     //   ������ѧ����������ˮƽ����ĸ߶�
	 XLccd = -(Wcar*0.5 + P)- Lsafe  ;     //   780.0������ѧ���ĵ�ˮƽλ��980.0
	 XRccd = +(Wcar*0.5 - P)+ Lsafe  ;
	 theta_x = (2*pi*(90 + x_angle))/360.0;    //  30 �� x�� ��ת�ĽǶ���
	 theta_y = (2*pi*(y_angle))/360.0;           //  -3 �� y�� ��ת�ĽǶ���
	 theta_z = (2*pi*(z_angle))/360.0;           //   �� z�� ��ת�ĽǶ���
...
void CUntitled::Lineff()
{
	...
	{
		xwf = XLccd + (i-1)*x_dif  ;
		ywf = -Zccd *  VTan_theta  +atsafe;                  //  adjust 390���ո����� 800   +191.8
		zwf = 0;
...
void CUntitled::ParkSevenLine()
{
	...
	for(int y=-1;y<=3;y++)
	{
		for(int x=1;x<=dotNum;x++)
		{
			...
			lcd_fig[yf_lcd[x]+y][xf_lcd[x]][0]=255; //һ
			if(pp == 0&&x>dotNum/2) {
					pBuf[0] =xf_lcd[x];
					pBuf[1] = yf_lcd[x]+y;
					pp+=2;
		//	__android_log_print(4,"BCR", "1. x = %d  y = %d", pBuf[0], pBuf[1]);
			...
			lcd_fig[ym_lcd[x]+y][xm_lcd[x]][0]=255;  //��           at1m
			if(fp == 0&& x==1)
				{
						pBuf[8] =xm_lcd[x];
						pBuf[9] = ym_lcd[x]+y;
			...
			lcd_fig[y1m_lcd[x]+y][x1m_lcd[x]][0]=255;  //��        at2m
			if(fp == 2&& x==dotNum)
				{
						pBuf[10] =x1m_lcd[x];
						pBuf[11] = y1m_lcd[x]+y;
			...
			lcd_fig[y2m_lcd[x]+y][x2m_lcd[x]][0]=255;  //��        at3m
			if(fp == 4&& x==1)
				{
						pBuf[12] =x2m_lcd[x];
						pBuf[13] = y2m_lcd[x]+y;
			...
			lcd_fig[y3m_lcd[x]+y][x3m_lcd[x]][0]=255;  //��            at4m
			if(pp == 2&&x>dotNum/2) {				
						pBuf[2] =x3m_lcd[x];
						pBuf[3] =y3m_lcd[x]+y;
						pp+=2;
			...
			for(int i=1;i<=ndotRstart/*100 dotNum*(at1m-Rstart_point)/(at3m-Rstart_point)*/;i++)
					{
						if( yr_lcd[i]+y < 0 || xr_lcd[i]+x < 0 )
							return;
						lcd_fig[yr_lcd[i]+y][xr_lcd[i]+x][0]=255;  //�ҵײ�
						if(pp == 4) {
								pBuf[4] =xr_lcd[i]+x;
								pBuf[5] =yr_lcd[i]+y;
...
void CUntitled::SampleLocusLine_L1(double angle, bool isSaveFile)
{
	...
	r_trk = Lcar/tan(delta);   //   Բ���뾶
		//LcL = Lcar;
	r_trk2 = LcL/tan(delta);   //   Բ���뾶
	alpha_max = SmaxRL/(r_trk+Wcar/2) +asin(Fcar/(r_trk+Wcar/2));    //   һ�������Ļ���
	alpha_max2 = SmaxRL/(r_trk2+Wcar/2) +asin(Fcar/(r_trk2+Wcar/2));	 //   һ�������Ļ���


		alpha0_L  = asin((Fcar + Lstart_point)/(r_trk-Wcar/2));
		alpha0_R  = asin((Fcar + Rstart_point)/(r_trk2+Wcar/2));
		dalpha_L = (alpha_max -alpha0_L)/(dotNum-1);    //   ���߽Ƕȷֱ���
			...
			for(int i=1; i<=dotNum;i++)
			{
				...
				for(int x=mB;x<=mA;x++)
				{
					for(int y=-1;y<=0;y++)
					{
						...
						{
						//	lcd_fig[yddr[i]+y][xddr[i]+x][1]=255;  //����
						lcd_fig[yddr[i]+y][xddr[i]+x][0]=255;  //����
						}

 						if(yddl[i]>(mMinY-7*N/50)&&yddl[i]+y<Nb) 
 						{ 
						//	lcd_fig[yddl[i]+y][(xddl[i]+x)][1]=255;  //����
							lcd_fig[yddl[i]+y][(xddl[i]+x)][0]=255;  //����
							if(mflag == 0)
							{	mBuf[2] = xddl[i];
								mBuf[3] = yddl[i];
...
void CUntitled::Test_ChangeParam_ParkLine(FuncParam funPm/*ParkLineParam lineparam,CarParam carP,CameraParam camP*/)
{
	...
	dotNum =lineparam.dotNum;       //  ����ĸ���
	Smax = lineparam.Smax;     //   ������Զ����
	SmaxRL = Smax + 1000;
	start_point =80;//lineparam.start; //  ���ҿ�ʼ���ߵ�λ��
	Lstart_point = 220;//lineparam.Lstart; //  ���ҿ�ʼ���ߵ�λ��
	...
	CameraParam camP = funPm.camP;
		// P  = camP.dP;
	  high = camP.nhigh;            //   ������ѧ����������ˮƽ����ĸ߶�
	 Vision = camP.nVision;          //  ������ӽ�  
	Pu = camP.dPu;             //  ��������ƫ��CCDˮƽ ʹͼ������Ϊ+  0.01
	Pv = camP.dPv;             //  ��������ƫ��CCD��ֱ ʹͼ������Ϊ+  0.01
	x_angle = camP.dx_angle;        //  �����X ����ˮƽ�Ƕ�
