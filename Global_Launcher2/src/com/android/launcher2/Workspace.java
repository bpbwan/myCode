...
public class Workspace extends SmoothPagedView
    ...
    private static final String STK1_NAME = "com.android.stk.StkLauncherActivity";
    private static final String STK2_NAME = "com.android.stk.StkLauncherActivity2";
	...
	/*
        ...
        */
		...
		mOverScrollPageIndex = -1; /* flyaudio  add for 8327*/
    ...
    protected void setWallpaperDimension() {
        ...
        if (LauncherApplication.isScreenLarge()) {
         ...
         //   mWallpaperHeight = maxDim;
         /* flyaudio  bpb  */
			mWallpaperWidth = 1024;
			mWallpaperHeight = 600;
        ...
        if (mWorkspaceFadeInAdjacentScreens &&
            ...
            for (int i = 0; i < getChildCount(); i++) {
                ...
                if (child != null) {
                    ...
                    child.getShortcutsAndWidgets().setAlpha(1f); //change 
                    if (!mIsDragOccuring) {
	...
	private void screenScrolledLargeUI(int screenCenter) {
		  ...
		  for (int i = 0; i < getChildCount(); i++) {
			  ...
			  if (cl != null) {
				  ...
				  if (!isSmall()) {
					  if ((mOverScrollX < 0 && i == 0)
							  || (mOverScrollX > mMaxScrollX && i == getChildCount() - 1)) {
						  isInOverscroll = true;
						  rotation *= -1;
				  ...
				  if (mFadeInAdjacentScreens && !isSmall()) {
					  float alpha = 1 - Math.abs(scrollProgress);
					  Log.d("DDD", "workspace setAlpha   "+alpha);
					  cl.setAlpha(alpha);
				  }
				  cl.invalidate();
			  }
  ...
  /* 		super.screenScrolled(screenCenter);
       ...
       */
       if (LauncherApplication.isScreenLarge()) {
            // We don't call super.screenScrolled() here because we handle the
            // adjacent pages alpha
            // ourselves (for efficiency), and there are no scrolling indicators
            // to update.
            screenScrolledLargeUI(screenCenter);
        } else {
            super.screenScrolled(screenCenter);
            // 2016.6.20
            // homeAnimation.ScreenScroll(screenCenter);
    ...
    /*    boolean small = mState == State.SMALL || mIsSwitchingState;
        ...
        */  //flyaudio  for 8327
        ...
        if (enableChildrenLayers != mChildrenLayersEnabled) {
            mChildrenLayersEnabled = enableChildrenLayers;
            // calling setChildrenLayersEnabled on a view that's not
            // visible/rendered
            // causes slowdowns on some graphics cards, so we only disable it
            // here and leave
            // the enabling to dispatchDraw
            if (!enableChildrenLayers) {
                for (int i = 0; i < getPageCount(); i++) {
                    ((ViewGroup) getChildAt(i)).invalidate();
                    Log.d("DDD", "------updateChildrenLayersEnabled-----"
                            + getPageCount());
                }
            }
        }
    ...
    */
    ...
    Animator getChangeStateAnimation(final State state, boolean animated, int delay) {
		...
		//Log.d("DDD", "mSpringLoadedShrinkFactor "+mSpringLoadedShrinkFactor);
...
/*  //flyaudio  bpb  dont  change the  background  2016626
    ...
    public static boolean thisisFlyButton = false;

    void startDrag(CellLayout.CellInfo cellInfo) {
        View child = cellInfo.cell;

		
        thisisFlyButton = false;
        Object tag = child.getTag();
        int del = ((ItemInfo) tag).delete;
        Log.d("DDD", "--------del---->" + del);
        if (del == 1) {
            thisisFlyButton = true;
        }
		
		if (tag instanceof ItemInfo) {
            // System.out.println( ((LauncherAppWidgetInfo) tag).toString());
            if (((ItemInfo) tag).move == 0) {
				Log.d("DDD", "--------move----> 0" );
                return;
            }
        }
	...
	// by kai 2013-08-26 增加主屏
	 public CellLayout addScreen() {
		 Launcher.cellFlag = true;
		 LayoutInflater layoutInflter = LayoutInflater.from(mLauncher);
		 CellLayout cl = (CellLayout) layoutInflter.inflate(
				 R.layout.workspace_screen, this, false);
		 int screen_leftpadding = mLauncher.getIntegerFromSkin(
				 Launcher.skinContext, "workspace_screen_leftpadding");
		 int screen_bottompadding = mLauncher.getIntegerFromSkin(
				 Launcher.skinContext, "workspace_screen_bottompadding");
		 int screen_toppadding = mLauncher.getIntegerFromSkin(
				 Launcher.skinContext, "workspace_screen_toppadding");
		 cl.setPadding(screen_leftpadding, screen_toppadding,
				 screen_leftpadding, screen_bottompadding);
		 addView(cl);
		 Launcher.SCREEN_COUNT++;
		 mLauncher.setScreenCount(Launcher.SCREEN_COUNT);
		 cl.setClickable(true);
		 cl.setOnTouchListener(this);// kai 6.25
		 cl.setOnInterceptTouchListener(this);
		 cl.setOnLongClickListener(mLongClickListener);
		 if (!mLauncher.isAllAppsVisible())
			 showIndicatorView(getCurrentPage());
		 Launcher.cellFlag = false;
		 return cl;
	 }
	
	 /**
	  * kai: Remove the specified screen and all the contents Almos update
	  * remaining screens content inside model
	  * 
	  * @param screen
	  */
	 protected void removeScreen() {
		 CellLayout layout = (CellLayout) getChildAt(getChildCount() - 1);
		 ViewGroup child = (ViewGroup) layout.getChildAt(0);
		 int childCount = child.getChildCount();
		 LauncherModel model = mLauncher.getModel();
		 Log.d("DDD", "workspace-removeScreen-childCount = " + childCount);
		 for (int j = 0; j < childCount; j++) {
			 final View view = child.getChildAt(j);
			 ...
			 if (item.container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
				 if (item instanceof LauncherAppWidgetInfo) {
					 ...
					 // item);
					 // model.deleteItemFromDatabase(mLauncher, item);
				 } else {
			 ...
			 if (item instanceof FolderInfo) {
				 ...
				 model.deleteFolderContentsFromDatabase(mLauncher,
						 userFolderInfo);
    ...
    public boolean acceptDrop(DragObject d) {
        ...
        if (d.dragSource != this) {
            ...
            if (!foundCell) {
				...
				Log.d("DDD", "mTargetCell[0] = " + mTargetCell[0]
    ...
    public void onDragExit(DragObject d) {
        ...
        if (mInScrollArea) {
            if (isPageMoving()) {
                ...
                Log.d("DDD", "mDropToLayout getPageAt");
                mDropToLayout = (CellLayout) getPageAt(getNextPage());
            } else {
             Log.d("DDD", "mDropToLayout mDragOverlappingLayout");
                mDropToLayout = mDragOverlappingLayout;
            }
        } else {
         Log.d("DDD", "mDropToLayout mDragTargetLayout");
    ...
    void setCurrentDropLayout(CellLayout layout) {
		...
		Log.d("DDD", "setCurrentDropLayout ");
    ...
    void setCurrentDragOverlappingLayout(CellLayout layout) {
		...
		Log.d("DDD", "setCurrentDragOverlappingLayout ");
   ...
   static private float squaredDistance(float[] point1, float[] point2) {
    ...
    public void onDragOver(DragObject d) {
	    ...
	    if (!Launcher.FLYMODE && mLauncher.getHotseat() != null && !isDragWidget(d)) {
                mLauncher.getHotseat().getHitRect(r);
                if (r.contains(d.x, d.y)) {
                    layout = mLauncher.getHotseat().getLayout();
                }
            ...
            if (layout != mDragTargetLayout) {
				Log.d("DDD", "  mLauncher.getHotseat");
                setCurrentDropLayout(layout);
                setCurrentDragOverlappingLayout(layout);
            }
        ...
        if (info instanceof PendingAddItemInfo) {
            ...
            case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
                if (info.container == NO_ID && info instanceof ApplicationInfo) {
                    ...
                    info.move = 1;
					info.delete = 0;
					Log.d("DDD", "onDropExternal Came from all apps -- make a copy ");
    ...
    protected void dispatchRestoreInstanceState(SparseArray<Parcelable> container) {
        ...
        // flyaudio 20160705
        Log.d("DDD", "dispatchRestoreInstanceState ");
    ...
    public boolean onEnterScrollArea(int x, int y, int direction) {
		...
		Log.d("DDD", "onEnterScrollArea +x "+x+" y "+y+" direction "+direction);
                ...
                if (tag instanceof ShortcutInfo) {
                    ...
                    if (info.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION &&
                        ...
                        for (int k = 0; k < appCount; k++) {
							...
							if (STK_PACKAGE_NAME.equals(name.getPackageName())
                            /* && tm.isMultiSimEnabled() */) {
                                /*
                                 * Log.d(TAG, "tm.hasIccCard(0)" +
                                 * msimtm.hasIccCard(0) + "tm.hasIccCard(1)" +
                                 * msimtm.hasIccCard(1));
                                 */
                                if (STK1_NAME.equals(name.getClassName())
                                /* && !msimtm.hasIccCard(0) */) {
                                    Log.d(TAG,
                                            "updateShortcuts removeItems stk1");
                                ...
                                /* && !msimtm.hasIccCard(1) */) {
                                    Log.d(TAG,
                                            " updateShortcuts removeItems stk2");
                                    LauncherModel.deleteItemFromDatabase(
                                            mLauncher, info);
                                    childrenToRemove.add(view);
                                }
                            ...
                            else if (app.componentName.equals(name)) {
