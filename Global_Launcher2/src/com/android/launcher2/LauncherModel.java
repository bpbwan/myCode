...
public class LauncherModel extends BroadcastReceiver {
    ...
    public interface Callbacks {
		...
		public void addAppInfo(ArrayList<ApplicationInfo> apps); //flyaudio bpb
    ...
    static void addOrMoveItemInDatabase(Context context, ItemInfo item, long container,
        ...
        if (item.container == ItemInfo.NO_ID) {
            // From all apps
            Log.d("DDD", "addOrMoveItemInDatabase ");
    ...
    static void moveItemInDatabase(Context context, final ItemInfo item, final long container,
        ...
        String transaction = "DbDebug    moveModify item (" + item.title + ") in db, id: " + item.id +
    ...
    static void addItemToDatabase(Context context, final ItemInfo item, final long container,
        ...
        item.updateValuesWithCoordinates(values, item.cellX, item.cellY);
		Log.d("DDD", "addItemToDatabase notify "+notify);
        Runnable r = new Runnable() {
            public void run() {
    ...
    public void onReceive(Context context, Intent intent) {
		...
		Log.d("DDD", "onReceive intent=" + intent);
        ...
        if (Intent.ACTION_PACKAGE_CHANGED.equals(action)
            ...
            forceReload();
        } else if ("FLY.ANDROID.NAVI.MSG.SENDER".equals(action)) {
            if ("1".equals(SystemProperties.get("fly.android.navi"))) {

                String nightMode = intent.getStringExtra("FLY_DAYNIGHT_MODE");

                // String nightMode = SystemProperties.get("fly.night.mode",
                // "0");

                if (BubbleTextView.DAY.equals(nightMode)) {
                    NIGHT = false;
                } else if (BubbleTextView.NIGHT.equals(nightMode)) {
                    NIGHT = true;
                }
                Log.d("launcher", "NIGHT:" + NIGHT);
                forceReload();
            }
        ...
        }else if ("com.qrd.thememgr.action.SWITCH_THEME".equals(action)) {
            ...
            final ContentValues values = new ContentValues();
            cr.update(Uri
                    .parse("content://com.android.launcher2.settings/resetdb"),
                    values, null, null);
            ...
            try {
                if (currentTheme.equals("default")) {
                    Log.e(TAG, "set wallpaper to default");
                    wpm.setResource(com.android.internal.R.drawable.default_wallpaper);
                } else {
                    Log.e(TAG, "change theme to red");
                    ...
                    ZipFile themeSrc = new ZipFile(themeBasePath + currentTheme
                            + ".zip");
                    /*
                     * Enumeration<? extends ZipEntry> e = themeSrc.entries();
                     * while(e.hasMoreElements()) Log.e(TAG, "zip entri" +
                     * ((ZipEntry)e.nextElement()).getName());
                     */
                    Log.e(TAG, "set wallpaper to theme");
                    InputStream stream = null;
                    String wallpaperPath = "Launcher/wallpaper.jpg";
...
*/
    public void startLoader(boolean isLaunching, int synchronousBindPage) {
        synchronized (mLock) {
			...
			Log.d("DDD", "startLoader "+synchronousBindPage+
				" mAllAppsLoaded "+mAllAppsLoaded+"  mWorkspaceLoaded "+mWorkspaceLoaded);
    ...
    private class LoaderTask implements Runnable {
        ...
        public void run() {
            ...
            keep_running: {
              ...
              //  }
				...
				//!Launcher.FLYMODE
             //   loadAndBindAllNotInDesktopApps();  /*   flyaudio  bpb 多余的app会add进shortcut */
			...
			Log.d("DDD", "loadAndBindAllApps over");
        ...
        private AlreadyInDesktopApp mAlreadyInDesktopApp;
        private void loadWorkspace() {
            final long t = DEBUG_LOADERS ? SystemClock.uptimeMillis() : 0;
            
            mAlreadyInDesktopApp = new AlreadyInDesktopApp();
         ...
         //   mApp.getLauncherProvider().loadDefaultFavoritesIfNecessary(0); //not need
            ...
            synchronized (sBgLock) {
                ...
                try {
                    ...
                    Log.d("DDD", "loadworkspace moveIndex");
					final int moveIndex = c
							.getColumnIndexOrThrow(LauncherSettings.Favorites.MOVE);
					
					final int delIndex = c
							.getColumnIndexOrThrow(LauncherSettings.Favorites.DELETE);
                    ...
                    while (!mStopped && c.moveToNext()) {
                        try {
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
								...
								//Log.d("DDD", " info "+info.componentName
                                 //                   .getClassName());
                                ...
                                if (info != null) {
                                 ...
                                 // flyaudio add
                                	...
                                	info.move = c.getInt(moveIndex);
                                	info.delete = c.getInt(delIndex);   
                      /* 			 if (container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
										mAlreadyInDesktopApp
												.addScreen(info.screen);
									}
						*/
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_FOLDER:
                                ...
                                folderInfo.cellY = c.getInt(cellYIndex);
                                                // flyaudio add
                            	folderInfo.move = c.getInt(moveIndex);
                            	folderInfo.delete = c.getInt(delIndex);            
                                if (container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
									mAlreadyInDesktopApp
											.addScreen(folderInfo.screen);
								}
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET:
                                ...
                                id = c.getLong(idIndex);
								Log.d("DDD", "info ITEM_TYPE_APPWIDGET ");
                                ...
                                if (!isSafeMode && (provider == null || provider.provider == null ||
                                    ...
                                    // flyaudio add
                                	appWidgetInfo.move = c.getInt(moveIndex);
                               	 	appWidgetInfo.delete = c.getInt(delIndex);
                                    mAlreadyInDesktopApp
									.addScreen(appWidgetInfo.screen);
									
									Log.d("DDD", " appWidgetInfo "+appWidgetInfo.providerName.getClassName());
                                    container = c.getInt(containerIndex);
                                    if (container != LauncherSettings.Favorites.CONTAINER_DESKTOP &&
                                        container != LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
                    ...
                    c.close();
					Log.d("DDD", " loadworkspace close Cursor");
                }
        ...
        private void loadAndBindAllApps() {
			...
			Log.d("DDD", "mAllAppsLoaded "+mAllAppsLoaded);
		...
		private ArrayList<ApplicationInfo> addCountInformation(Context mContext,
				  ArrayList<ApplicationInfo> list) {
				  Log.d("DDD", "addCountInformation");
		/*	  Cursor c = null;
			  try {
				  c = mContext.getContentResolver().query(LauncherInfo.CONTENT_URI,
					  ...
					  do {
						  for (int j = 0; j < count; j++) {
							  if (c.getString(0).equals(
								  ...
								  Log.d("DDD", "j "+j+" count "+list.get(j).count);
								  continue;
							  }
						  }
					  } while (c.moveToNext());
		...
		*/
		...
		private void bindAppInfo(Context mContext, ArrayList<ApplicationInfo> list){ /* flyaudio bpb */
				final Callbacks oldCallbacks = mCallbacks.get();
				if (oldCallbacks == null) {
					return;
				}
				 ...
				 final Callbacks callbacks = tryGetCallbacks(oldCallbacks);
					SharedPreferences mSharedPreferences = mContext.getSharedPreferences("launcher_info", 0);
					 boolean firstLoad = mSharedPreferences.getBoolean(
									"firstload", true);
						   if (firstLoad) {
								Log.d("AAA", "		 bindAppInfo	flySortApp");
								try {
									if(callbacks != null)
										callbacks.addAppInfo(list);
								} catch (Exception e) {
			...
			while (i < N && !mStopped) {
				...
				if (added.size() > 0 && blankScreens.size() > 0) {
					for (int n = 0; n < added.size(); n++) {
						...
						shortcuts.add(sInfo);
						Log.d("DDD", "loadNotInDesktopAllAppsByBatch ");
        ...
        public void run() {
            ...
            if (mBgAllAppsList.removed.size() > 0) {
                ...
                for (int i = 0; i < N; ++i) {
                    ...
                    for (int k = 0; k < mBgAllAppsList.data.size(); k++) {
                        ApplicationInfo info = mBgAllAppsList.data.get(k);
                        String pn = info.getPackageName();
                        if (packages[i].equals(pn)) {
                            isExisted = true;
                            break;
                        }
                    }
            ...
            if (added != null) {
				...
				/* flyaudio  bpb 2016628   apps add not in workspace*/
	...
	static void insertHashMap(Context mcontext,HashMap<Object, byte[]> cache, ItemInfo info){
		...
		try {
		final int iconIndex = c.getColumnIndexOrThrow(LauncherSettings.Favorites.ICON);
		Log.d("DDD", "insertHashMap iconIndex "+iconIndex);
		if(info instanceof ShortcutInfo) {
			
			insertHashMap(cache, (ShortcutInfo)info, c, iconIndex);
			}
			}catch(Exception e){
				Log.w(TAG, "Desktop items loading interrupted:", e);
			}
	...
	static void insertHashMap(HashMap<Object, byte[]> cache, ShortcutInfo info, Cursor c,
            int iconIndex) {
    ...
    public static final Comparator<ApplicationInfo> APP_COUNT_COMPARATOR = new Comparator<ApplicationInfo>() {
        public final int compare(ApplicationInfo a, ApplicationInfo b) {
            int result = 0;
            if (a.count != b.count) {
                result = b.count - a.count;
            } else {
                result = sCollator.compare(a.title.toString(),
                        b.title.toString());
                if (result == 0) {
                    result = a.componentName.compareTo(b.componentName);
                }
