...
import com.android.launcher2.util.FavoritesEntity;
import com.android.launcher2.util.FavoritesXMLParser;
import com.android.launcher2.util.XMLTool;
...
public class LauncherProvider extends ContentProvider {
    ...
    static final String SETMODE = "current_mode";// issaclam edit
	...
	public static DatabaseHelper staticOpenHelper = null;

    @Override
    public boolean onCreate() {
    	Log.d("DDD", "DatabaseHelper onCreate");
        mOpenHelper = new DatabaseHelper(getContext());
		staticOpenHelper = mOpenHelper;
    ...
    public SQLiteDatabase getSQLiteDatabaseFromLauncherProvider() {
    ...
    private static long dbInsertAndCheck(DatabaseHelper helper,
        ...
        if (TABLE_FAVORITES.equals(table)
                && !values.containsKey(LauncherSettings.Favorites._ID)) {
	...
	public static boolean isDataBaseOpen(){
		if(staticOpenHelper!=null){
		//	SQLiteDatabase db = staticOpenHelper.getWritableDatabase();
			staticOpenHelper.close();
		return false;
			}
		return false;
	}
    ...
    public int update(Uri uri, ContentValues values, String selection,
            ...
            Log.d("DDD", "update ContentResolver");
        if (uri.toString().equals(
                "content://com.android.launcher2.settings/resetdb")) {
            Log.e(TAG, "reset launcher DB");
            SQLiteDatabase db = mOpenHelper.getWritableDatabase();
            try {
    ...
    synchronized public void loadDefaultFavoritesIfNecessary(int origWorkspaceResId) {
        ...
        if (sp.getBoolean(DB_CREATED_BUT_DEFAULT_WORKSPACE_NOT_LOADED, false)) {
			...
			Log.d("DDDD", "  loadDefaultFavoritesIfNecessary origWorkspaceResId"+origWorkspaceResId);
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL("CREATE TABLE favorites (" + "_id INTEGER PRIMARY KEY,"
                    ...
                    + "uri TEXT," + "displayMode INTEGER," + "move INTEGER,"
                    + "flydelete INTEGER" + ");");

            db.execSQL("CREATE TABLE launcher_info ("
                    + "_id INTEGER PRIMARY KEY," + "package_name TEXT,"
                    + "class_name TEXT," + "title TEXT,"
                    + "launch_count INTEGER DEFAULT 0" + ");");

            // ///////////////issaclam edit
            db.execSQL("CREATE TABLE current_mode ("
                    + "_id INTEGER PRIMARY KEY," + "ch_mode TEXT,"
                    + "en_mode TEXT," + "mode INTEGER," + "current TEXT" + ");");
			...
			try {
                    String currentTheme = SystemProperties.get(
                            "persist.sys.qrd_theme.current", "default");
                    if (!currentTheme.equals("default")) {
                        ...
                        ZipFile themeSrc = new ZipFile(themeBasePath
                                + currentTheme + ".zip");
                        InputStream stream = null;
                        String hotseat_config = "Launcher/hotseat.xml";
                        ZipEntry hotseatEntry = themeSrc
                                .getEntry(hotseat_config);
                        if (hotseatEntry != null) {
                            stream = themeSrc.getInputStream(hotseatEntry);
                            loadFavoritesByPath(db, stream);
                            android.util.Log.e("filePathInThemeHotSeat",
                                    "use theme");
                        }
                        themeSrc.close();
                    } else {
                    
						 String path = "/data/fly_widget.xml";
                        if (new File(path).exists()) {
                            File file = new File(path);
                            InputStream is = new FileInputStream(file);
                            loadFavoritesByPath(db, is);
                        } else {
                        	Log.d("DDD"," provider loadFlyaudioFavorites");
                            // loadFavorites(db, R.xml.default_workspace);
                            loadFlyaudioFavorites(db);
                        	}
					}
				 }catch (Exception e) {
                    android.util.Log.e("filePathInThemeHotSeat Exception", ""
                            + e);
					}
            	}
		...
		private int loadFavoritesByPath(SQLiteDatabase db, InputStream stream) {
				   Intent intent = new Intent(Intent.ACTION_MAIN, null);
				   intent.addCategory(Intent.CATEGORY_LAUNCHER);
				   ContentValues values = new ContentValues();
				Log.d("DDD", "loadFavoritesByPath stream");
				   PackageManager packageManager = mContext.getPackageManager();
				   int i = 0;
				   try {
					   XmlPullParser parser = null;
					   // FileInputStream fis = new FileInputStream(workspaceResPath);
					   ...
					   while (eventType != XmlPullParser.END_DOCUMENT) {
						   ...
						   case XmlPullParser.START_TAG:
							   boolean added = false;
							   String launcherXMLNS = "http://schemas.android.com/apk/res/com.android.launcher";
							   final String name = parser.getName();
							   long container = LauncherSettings.Favorites.CONTAINER_DESKTOP;
							   if (parser
									   .getAttributeValue(launcherXMLNS, "container") != null)
								   container = Long.valueOf(parser.getAttributeValue(
										   launcherXMLNS, "container"));
		
							   String screen = parser.getAttributeValue(launcherXMLNS,
									   "screen");
							   String x = parser.getAttributeValue(launcherXMLNS, "x");
							   String y = parser.getAttributeValue(launcherXMLNS, "y");
							   String span_x = parser.getAttributeValue(launcherXMLNS,
									   "spanX");
							   String span_y = parser.getAttributeValue(launcherXMLNS,
									   "spanY");
							   String clsName = parser.getAttributeValue(
									   launcherXMLNS, "className");
							   String pkgName = parser.getAttributeValue(
									   launcherXMLNS, "packageName");
							
							   android.util.Log.e("LauncherDBInit", "tag is:" + name
									   + " pkg is:" + pkgName + " cls is:" + clsName
									   + " scrn is:" + screen + " cellx is:" + x
									   + " celly is:" + y + " spx is:" + span_x
									   + " spy is:" + span_y);
		
							   // If we are adding to the hotseat, the screen is used
							   // as the position in the
							   // hotseat. This screen can't be at position 0 because
							   // AllApps is in the
							   // zeroth position.
							   if (container == LauncherSettings.Favorites.CONTAINER_HOTSEAT
									   && Hotseat.isAllAppsButtonRank(Integer
											   .valueOf(screen))) {
								   throw new RuntimeException(
							   ...
							   Log.d("DDD", "pkgName "+pkgName);
								   long id = addAppShortcutExt(db, values, null,
										   pkgName, clsName, packageManager, intent);
								   added = id >= 0;
								   ...
								   added = addAppWidgetExt(db, values, null, pkgName,
										   clsName, span_x, span_y, packageManager, 3);
							   }
        ...
        private ArrayList<FavoritesEntity> dataList = new ArrayList<FavoritesEntity>();

        public int loadFlyaudioFavorites(SQLiteDatabase db) {
            Intent intent = new Intent(Intent.ACTION_MAIN, null);
            intent.addCategory(Intent.CATEGORY_LAUNCHER);
            ContentValues values = new ContentValues();
            Log.d("DDD", "loadFlyaudioFavorites");
            PackageManager packageManager = mContext.getPackageManager();
            try {

                String path = SystemProperties.get("persist.fly.car.select",
                        "default");
                // tangnengwu
                // if(isHaveShortcutConfig())
                path = "flysystem/flyconfig/" + path
                        + "/uiconfig/default_workspace.xml";
                // else
                // path =
                // "flysystem/flyconfig/"+path+"/uiconfig/default_workspace.xml";
                InputStream inStream = new FileInputStream(new File(path));
                FavoritesXMLParser parser = new FavoritesXMLParser();
                Log.i("@@@@@@@", path);
                dataList = ((FavoritesXMLParser) XMLTool
                        .parse(inStream, parser)).getData();

                Log.d("lp", "dataList:" + dataList.size());

                for (FavoritesEntity entity : dataList) {
                    final String name = entity.getType();
                    ...
                    values.put(LauncherSettings.Favorites.SCREEN,
                            entity.getScreen());
                    values.put(LauncherSettings.Favorites.CELLX, entity.getX());
                    values.put(LauncherSettings.Favorites.CELLY, entity.getY());
                    // flyaudio add
                    values.put(LauncherSettings.Favorites.MOVE,
                            entity.getMove()); // 可移动
                    values.put(LauncherSettings.Favorites.DELETE,
                            entity.getDelete());

                    if (TAG_FAVORITE.equals(name)) {
						
                        flyaddAppShortcutExt(db, values, entity, null, null,
                                packageManager, intent);
                    } else if (TAG_SEARCH.equals(name)) {
                    Log.d("DDD", "TAG_SEARCH");
                        addSearchWidget(db, values);
                    } else if (TAG_CLOCK.equals(name)) {
                    Log.d("DDD", "TAG_CLOCK");
                        addClockWidget(db, values);
                    } else if (TAG_APPWIDGET.equals(name)) {
                     Log.d("DDD", "TAG_APPWIDGET");
                        flyaddAppWidgetExt(db, values, entity, null, null,
                                null, null, packageManager);
                    } else if (TAG_SHORTCUT.equals(name)) {
                        flyaddUriShortcut(db, values, entity);
                    }

                }

            } catch (Exception e) {
                Log.w(TAG, "Got exception parsing flyaudio favorites.", e);
                // Toast.makeText(mContext, "解析飞歌快捷方式异常!", Toast.LENGTH_LONG)
                // .show();
                 Log.w("DDD", " provider load R.xml.default_workspace", e);
                loadFavorites(db, R.xml.default_workspace);
            }
            return dataList.size();
        }

		private long flyaddAppShortcutExt(SQLiteDatabase db,
					   ContentValues values, FavoritesEntity entity, String pkgName,
					   String clsName, PackageManager packageManager, Intent intent) 
					   {
				   long id = -1;
				   ActivityInfo info;
				   String packageName = entity.getPackageName();
				   String className = entity.getClassName();
				Log.d("DDD", "FAVORITE "+packageName);
				   try {
					   ComponentName cn;
					   try {
					   ...
					   values.put(Favorites.DELETE, entity.getDelete());
		 ...
		 private long addAppShortcutExt(SQLiteDatabase db, ContentValues values,
                TypedArray a, String pkgName, String clsName,
		...
		private boolean flyaddAppWidgetExt(SQLiteDatabase db,
						ContentValues values, FavoritesEntity entity, String pkgName,
						String clsName, String strSpanX, String strSpanY,
			...
			Log.d("DDD", "WidgetExt "+packageName);
					...
					if (hasPackage) {
						int spanX, spanY, dele;
						spanX = Integer.parseInt(entity.getSpanX());
						spanY = Integer.parseInt(entity.getSpanY());
						dele = Integer.parseInt(entity.getDelete());
						return addAppWidget(db, values, cn, spanX, spanY, dele);
					}
		
					return false;
				}
		private long flyaddUriShortcut(SQLiteDatabase db, ContentValues values,
					 FavoritesEntity entity) {
				 Resources r = mContext.getResources();
				 final int iconResId = Integer.parseInt(entity.getIcon());
				 final int titleResId = Integer.parseInt(entity.getTitle());
        ...
        private boolean convertDatabase(SQLiteDatabase db) {
			...
			Log.d("DDD", "convertDatabase "+converted);
            return converted;
        }
        ...
        private int copyFromCursor(SQLiteDatabase db, Cursor c) {
			...
			// flyaudio add
			final int moveIndex = c.getColumnIndexOrThrow(LauncherSettings.Favorites.MOVE);
			
			final int delIndex = c.getColumnIndexOrThrow(LauncherSettings.Favorites.DELETE);
			...
			//fly add
				values.put(LauncherSettings.Favorites.MOVE, c.getInt(moveIndex));
                values.put(LauncherSettings.Favorites.DELETE, c.getInt(delIndex));
                rows[i++] = values;
            }
        ...
        private int loadFavorites(SQLiteDatabase db, int workspaceResourceId) {
            ...
            try {
                ...
                while (((type = parser.next()) != XmlPullParser.END_TAG ||
					...
					values.put(LauncherSettings.Favorites.DELETE, 3);
						  // flyaudio add
					values.put(LauncherSettings.Favorites.MOVE,
								  a.getInt(R.styleable.Favorite_move, 1)); // 可移动
					
					values.put(LauncherSettings.Favorites.DELETE,
								  a.getInt(R.styleable.Favorite_flydelete, 1)); // 可
																				// 删除
		...
		private boolean addAppWidgetExt(SQLiteDatabase db,
					   ContentValues values, TypedArray a, String pkgName,
					   String clsName, String strSpanX, String strSpanY,
					   PackageManager packageManager, int del) {
				   ...
				   if (hasPackage) {
					   ...
					   if (a != null) {
						   ...
						   spanX = Integer.valueOf(strSpanX);
						   spanY = Integer.valueOf(strSpanY);
					   }
					   return addAppWidget(db, values, cn, spanX, spanY, del);
				   }
		...
		private boolean addAppWidget(SQLiteDatabase db, ContentValues values,
					   ComponentName cn, int spanX, int spanY, int del) {
				   ...
				   try {
					   ...
					   values.put(Favorites.DELETE, del);
		 			   values.put(Favorites.MOVE, del);
