...
public class CellLayout extends ViewGroup {
	...
	private int workTop = -1;
	private int appsLeft = 35;
	private int appsTop = 30;
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		Log.i("@@@@1", "mCellHeight="+mCellHeight);

		if(Launcher.cellFlag)
			mCellHeight = mOriginalCellHeight = Launcher.flyLauncher.
			  getIntegerFromSkin(Launcher.skinContext, "workspace_cell_height");
    ...
    public void setCellWidth(int param)
    {
    	mCellWidth=param;
    	invalidate();
    }
	...
	/*
		for(int i = 0, msize = states.size(); i < msize; i++) 
			{ 
			Parcelable p_state = states.valueAt(i); 
			
		 if(p_state == BaseSavedState.EMPTY_STATE && p_state != null) {
		 	Log.d("DDD", "states BaseSavedState.EMPTY_STATE "+msize);
			return;
			}
		}
		*/
    ...
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params,
        ...
        if (child instanceof BubbleTextView) {
			...
			workLeft = (mCellWidth-bubbleChild.Width)/2;
			workTop = (mCellHeight - bubbleChild.Height)/2;
		...
		//	Drawable bg = getResources().getDrawable(R.drawable.myimg);  /* flyaudio test */
	...
	//		Log.d("DDD", "mWidthGap "+mWidthGap+"Width "+mOriginalCellWidth+" hSpace "+hSpace
//				+" hFreeSpace "+hFreeSpace+" vFreeSpace "+vFreeSpace+" vSpace "+vSpace+
	//			"mHeightGap  "+mHeightGap+" mOriginalCellHeight "+mOriginalCellHeight);
	//		mCellHeight = 200;

//		Log.d("DDD", " heightSpecSize "+heightSpecSize+" top "+getPaddingTop()+" bottom "+getPaddingBottom());
            mShortcutsAndWidgets.setCellDimensions(mCellWidth, mCellHeight, mWidthGap, mHeightGap);
        } else {
    ...
    void visualizeDropLocation(View v, Bitmap dragOutline, int originX, int originY, int cellX,
		...
		if(Launcher.FLYMODE)
			dragRegion = null;  //for center dragoutline  flyaudio add 
        ...
        if (cellX != oldDragCellX || cellY != oldDragCellY) {
			...
			if(Launcher.FLYMODE) /* flyaudio 8327  bpb*/ {
				if(Launcher.LongDragMode == Launcher.WORKSPACE_LONGCLICK_MODE) {
					left+=workLeft;
					top+= workTop;
				}
				else if(Launcher.LongDragMode == Launcher.APPCUSTOM_LONGCLICK_MODE) {
					left+=appsLeft;
					top+=appsTop;
				}
			}
