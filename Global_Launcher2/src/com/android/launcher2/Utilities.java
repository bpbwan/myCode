...
final class Utilities {
	...
	public static boolean getBoolFromSkin(Context skinPackageContext,String resourceName)
		{
			int id=0;
			try {
				id=skinPackageContext.getResources().getIdentifier(resourceName,
						"bool", "com.flyaudio.skin");
			} catch (Exception e) {
    ...
    static Bitmap createIconBitmap(Drawable icon, Context context) {
           synchronized (sCanvas) { // we share the statics :-(
            ...
            //读进配置文件
            if(Launcher.flyLauncher==null||Launcher.skinContext==null)
            {
            	if(Launcher.flyLauncher==null)
            		Log.i("@@@1", "Launcher.flyLauncher==null");
            	else
            	{
            		Log.i("@@@1", "Launcher.skinContext==null");
            	}
            }
            
            boolean flag=
            		getBoolFromSkin(Launcher.skinContext, "app_icon_need_bg");
			...
			if (flag) {
					...
					Drawable src = 
							getDrawableFromSkin(Launcher.skinContext, "app_bg");
					BitmapDrawable bd = (BitmapDrawable) src;
					Bitmap	backBitmap = bd.getBitmap();
							
					int backWidth = backBitmap.getWidth();
					int backHeight = backBitmap.getHeight();
					Log.d(TAG, "backWidth == "+backWidth+"	 backHeight = "+backHeight);
					...
					canvas.drawBitmap(Bitmap.createBitmap(backBitmap, 0, 0, backWidth, backHeight, matrix, true), .0f, 0.0f, null);
					} else {
    ...
    static void updateLaunchInfo(ContentResolver contentResolver, String packageName, String className){
        ...
        long count = 0;
        try{
            c = contentResolver.query(LauncherInfo.CONTENT_URI, projection, selection, selectionArgs, null);
