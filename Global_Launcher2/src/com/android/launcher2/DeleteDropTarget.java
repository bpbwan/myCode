...
public class DeleteDropTarget extends ButtonDropTarget {
    ...
    private ApplicationInfo mDragApplicationInfo;
...
//        mUninstallDrawable = (TransitionDrawable) Launcher.flyLauncher
//                .getDrawableFromSkin(Launcher.skinContext,
//                        "uninstall_target_selector");
        
//        mRemoveDrawable = (TransitionDrawable) Launcher.flyLauncher
//                .getDrawableFromSkin(Launcher.skinContext,
//                        "remove_target_selector");

        if (LauncherModel.NIGHT) {
            
            mRemoveDrawable = (TransitionDrawable) Launcher.flyLauncher
                    .getDrawableFromSkin(Launcher.skinContext,
                            "remove_target_selector_night");
            mUninstallDrawable = (TransitionDrawable) Launcher.flyLauncher
                    .getDrawableFromSkin(Launcher.skinContext,
                            "uninstall_target_selector_night");
        } else {
            mRemoveDrawable = (TransitionDrawable) Launcher.flyLauncher
                    .getDrawableFromSkin(Launcher.skinContext,
                            "remove_target_selector");
	...
	/*	mDragApplicationInfo = null;
		if (info instanceof ShortcutInfo) {
			LauncherModel model = mLauncher.getModel();
			mDragApplicationInfo = model
					.findApplicationInfoFromAllAppList((ShortcutInfo) info);
		}

		return (mDragApplicationInfo != null);
	*/
    ...
    public void onDragStart(DragSource source, Object info, int dragAction) {
        ...
        /*
         * 设置allappsbutton与deleteicon的动画效果
         */
       
        boolean isVisible = true;
        boolean isUninstall = false;
        boolean isAllapp = false;
		Log.d("DDD", "onDragStart droptarget");
        ...
        mCurrentDrawable.resetTransition();
        setTextColor(mOriginalTextColor);
        Log.i("DDDT", "remove or uninstall isVisible  " + isVisible);
        ...
        if (getText().length() > 0) {
		 ...
		 Hotseat.mAllAppsButton.setVisibility(View.GONE); //use for not see the pageview->IndicatorView
		  if (!isAllapp) {
            if (Workspace.thisisFlyButton) {
                Hotseat.mAllAppsButton.setVisibility(View.VISIBLE);
    ...
    private ObjectAnimator mAllAppsButtonFadeInAnim;
    ...
    public void onDragEnd() {
	    ...
	    */
		...
		Hotseat.mAllAppsButton.setVisibility(View.VISIBLE);
	//	mAllAppsButtonFadeInAnim = ObjectAnimator.ofFloat(Hotseat.mAllAppsButton, "alpha", 0f, 1f);
    //    mAllAppsButtonFadeInAnim.setDuration(sTransitionInDuration);
	//	mAllAppsButtonFadeInAnim.start();
    ...
    private void completeDrop(DragObject d) {
        ...
        if (isApplicationInfoShortcut(d.dragSource, item)) {
			/*    flyaudio   */
